...
import (
	...
	"github.com/rancher/rancher-operator/pkg/namespaces"
...
const (
	...
	staticFeatures     = "multi-cluster-management=false,fleet=false,legacy=false"
)
...
func Register(ctx context.Context, clients *clients.Clients) {
	...
	rocontrollers.RegisterClusterGeneratingHandler(ctx, clients.Cluster.Cluster(),
		clients.Apply.
	...
	relatedresource.Watch(ctx, "manager-rancher-watch", h.resolveClusters,
		clients.Cluster.Cluster(), clients.Core.ConfigMap())
}
...
func (h *handler) values(cluster *rancherv1.Cluster) (map[string]interface{}, error) {
	reg, err := settings.Get(h.settings, "system-default-registry")
	if err != nil {
		return nil, err
	}
			...
			Targets: []v1alpha1.BundleTarget{
				{
					ClusterSelector: &metav1.LabelSelector{
						MatchExpressions: []metav1.LabelSelectorRequirement{
							...
							{
								Key:      "rancher.cattle.io/unmanaged-rancher",
